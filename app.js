...
//Configurações
    ...
    //Handlebars
    app.engine('handlebars', handlebars({deafultLayout: 'main' }))
...
app.get("/categorias", eUser, (req,res) => {
    Categoria.find().then((categorias) => {
        res.render("categorias/index", {"categorias": JSON.parse(JSON.stringify(categorias))})
    }).catch((err) => {
        req.flash("error_msg", "Houve umm erro interno ao listar as categorias")
        res.redirect("/")
    })
})
...
app.get("/categorias/:slug", eUser, (req,res) => {
    Categoria.findOne({slug:req.params.slug}).then((categoria) => {
        ...
        req.flash("error_msg", "Houve um erro interno ao cerregar a página desta categoria")
        res.redirect("/")
    })
})
